      ...
      <section id="pricing" className="py-24 px-6 bg-[#0b0b0b]">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {pricing.map((plan, i) => (
              <div key={i} className={`relative p-8 rounded-2xl border transition-all duration-300 ${plan.popular ? 'bg-gradient-to-b from-yellow-500/5 to-neutral-950 border-yellow-500/50 scale-105 shadow-2xl shadow-yellow-500/10' : 'bg-[#0f0f0f] border-[#1a1a1a] hover:border-[#2a2a2a]'}`}>
                {plan.popular && (
               ...
               {plan.price}
                ...
                <a 
                  ...
                  className={`w-full inline-flex justify-center items-center py-4 rounded-xl font-bold transition-all ${plan.popular ? 'bg-yellow-500 hover:bg-yellow-400 text-black shadow-lg shadow-yellow-500/20' : 'bg-[#141414] border border-neutral-800 hover:bg-[#1a1a1a] text-white'}`}
                >
                  {plan.cta}
                </a>
